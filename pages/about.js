...
function LinkButton(props) {
  return (
    <Link href={props.url}>
      <div
        className={`cursor-pointer mr-3 bg-white hover:bg-stone-700 hover:text-white border border-black text-black font-bold py-3 px-12 lg:px-8 duration-200 transition-colors mb-4 ${props.className}`}
      >
        {props.name}
      </div>
    </Link>
...
export default function About() {
  return (
    <>
      <div
        className={
          "relative container mx-auto px-5 py-16 max-w-3xl dark:text-white"
        }
      >
        ...
        <header className="mb-8">
          ...
          <h2 className="mb-2 text-1xl md:text-2xl font-bold tracking-tight leading-tight">
            Software Engineer
          </h2>
          <p>
            A software engineer living in Vancouver. Former engineer at Tencent,
            Thoughtworks and IBM. He loves anything that draws pictures using
            computers😇. He also loves StarCraft.
          </p>
        </header>
        ...
        <Section>
          ...
          />
          <Job
            title={"Software Consultant, Thoughtworks"}
            startYear={2019}
            endYear={2021}
          />
          <Job
            title={"Software Engineer, IBM Canada"}
            startYear={2016}
            endYear={2018}
          />
        </Section>
        ...
        <Section>
          ...
          <div className="mb-4 flex flex-row justify-left items-center flex-wrap">
            {PROJECT_LINKS.filter((item) => item.tag === TAGS.WEB3D).map(
              ({ name, url }, index) => (
                <LinkButton name={name} url={url} key={index} />
              )
            )}
          </div>
          <h3 className="mb-4 text-lg md:text-1xl tracking-tighter leading-tight">
          ...
          <div className="mb-4 flex flex-row justify-left items-center flex-wrap">
            {PROJECT_LINKS.filter((item) =>
              [TAGS.TOOL, TAGS.WEBAPP].includes(item.tag)
            ).map(({ name, url }, index) => (
              <LinkButton name={name} url={url} key={index} />
            ))}
          </div>
          ...
          <div className="mb-4 flex flex-row justify-left items-center flex-wrap">
            {PROJECT_LINKS.filter((item) => item.tag === TAGS.LAB).map(
              ({ name, url }, index) => (
                <LinkButton name={name} url={url} key={index} />
              )
            )}
